...
function getByPath(path, callback, FileSystem) {
    ...
    this.path = this.fs.mainPath + utils.relativePath(path, '').replaceAll('//', '/')
...
getByPath.prototype = {
    getFile: function() {
        ...
        if (this.isFile) {
            ...
            var folder = utils.stripOffFile(folder)
            try {
...
FileSystem.prototype = {
    ...
    writeFile: function(path, data, callback, allowOverWrite) {
        ...
        var path = utils.relativePath(path, '')
        var origpath = path
        var path = this.mainPath + path
        var path = path.replaceAll('//', '/').replaceAll('\\', '/')
        var folder = utils.stripOffFile(path)
